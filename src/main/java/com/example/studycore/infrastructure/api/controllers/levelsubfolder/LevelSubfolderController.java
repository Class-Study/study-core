...
import com.example.studycore.application.usecase.levelsubfolder.CreateSubfoldersBatchUseCase;
...
public class LevelSubfolderController implements LevelSubfolderApi {
    ...
    private static final LevelSubfolderInfraMapper MAPPER = LevelSubfolderInfraMapper.INSTANCE;
    ...
    @Override
    public ResponseEntity<Void> createBatch(UUID profileId, UUID folderId, CreateSubfoldersBatchRequest request) {
        createSubfoldersBatchUseCase.execute(
                MAPPER.toBatchInput(profileId, folderId, request, getAuthenticatedUserId())
        );
    ...
    public ResponseEntity<LevelSubfolderResponse> update(UUID profileId, UUID folderId, UUID subfolderId, UpdateLevelSubfolderRequest request) {
        final var input = new UpdateLevelSubfolderInput(
                profileId, folderId, subfolderId, request.name(), request.position(), getAuthenticatedUserId()
        );
        return ResponseEntity.ok(MAPPER.toResponse(updateLevelSubfolderUseCase.execute(input)));
    }
